...
# core-http reload), so this is the test that would have caught them.
#
...
check "core-http nginx reloaded so it loads the new pair" "grep -q 'exec -T core-http nginx -s reload' '$FAKE_DOCKER_LOG'"
check "core is never restarted" "! grep -qE 'restart( [a-z-]+)* core( |\$)' '$FAKE_DOCKER_LOG' && ! grep -q 'restart core-http' '$FAKE_DOCKER_LOG'"
...
check "fresh lineages: nothing reinstalled" "! grep -q 'core-http' '$FAKE_DOCKER_LOG'"
...
check "core-http reloaded after reinstall" "grep -q 'exec -T core-http nginx -s reload' '$FAKE_DOCKER_LOG'"
