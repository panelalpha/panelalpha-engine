...
/**
 * Git checkout and repository helpers for DinD source ingest (Lib Git remains production for panel API).
 */
class GitRepository
{
    use FastForwardPull;
    ...
    private const BACKUP_REF = 'refs/panelalpha/backup';
    ...
    public function __construct(
        ...
        if ($trimmed === '') {
            ...
            $this->absolutePath = $this->resolvePath($home, $trimmed);
        }
     ...
     */
    private function installAskPass(System $system, string $token, array $gitCommand): array
    {
    ...
    private function cleanupAskPass(System $system, ?string $askPassTemp, ?string $askPassPath): void
    {
        if ($askPassTemp !== null) {
            @unlink($askPassTemp);
        }
    ...
    private function resolvePath(string $home, string $trimmed): string
    {
