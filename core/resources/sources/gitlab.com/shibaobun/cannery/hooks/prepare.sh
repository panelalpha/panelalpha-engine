...
# ~/.panelalpha/cannery/ survives; ~/project is emptied on every deploy
...
STORE_DIR="${HOME}/.panelalpha/cannery"
...
DATABASE_URL=ecto://cannery:${PG_PASSWORD}@db/cannery
EOF
        cat > "${NOTE}" <<'EOF'
Cannery on this account
=======================

Cannery is a self-hosted firearm/ammunition inventory tracker. There is no
seeded administrator, and that is deliberate: Cannery makes the FIRST registered
user an admin, and closes public registration once an admin exists.

FIRST RUN
  1. Open this account's URL. Cannery's home page loads.
  ...
  3. Confirm your email (see EMAIL below) -- Cannery will NOT let you log in
     until the account is confirmed.
  4. Log in and start adding containers, ammo types and packs.
...
EMAIL (REQUIRED to finish onboarding)
  Cannery refuses login until the account's email is confirmed, and it confirms
  by emailing a one-time link. DISABLE_EMAIL=true by default, so with no SMTP
  the confirmation (and password-reset) messages are written to the app
  container log instead of being sent. Two supported ways to confirm:
    ...
    (b) One-person instance without SMTP -- read the link once from the log:
           docker compose logs cannery | grep users/confirm
...
SECRETS
  ...
  ~/.panelalpha/cannery/ (0600). They are generated once and never rewritten; a
...
# runtime.exs default; options en_US, de, fr, es.
